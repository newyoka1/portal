#!/usr/bin/env python3
"""
UPDATED NYS Voter Tagging Pipeline - Direct StateVoterId Matching
- Uses StateVoterId/VoterKey for direct matching (NO MORE MATCH KEYS!)
- Keeps ALL columns from audience CSV files
- Handles both full schema files and minimal files (like DONORS TO CONS with only VoterKey)
- Partitioned tables (32 partitions by CDName)
- Transaction batching for resumability

KEY CHANGES:
- Direct JOIN on StateVoterId (or VoterKey → StateVoterId lookup)
- No firstname|lastname|zip5|birthyear match key computation
- Stores all audience file columns dynamically
- Much faster and more accurate matching
"""
import os
import sys
import time
...
def create_stg_voter_raw(conn, header: list[str]):
    """UPDATED: No match keys needed - using StateVoterId"""
    cols = []
    for c in header:
    ...
    exec_sql(
        conn,
        f"CREATE TABLE stg_voter_raw ({', '.join(cols)}, KEY idx_statevoterid (StateVoterId(50))) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;"
    )
...
def rebuild_tables(conn, fullvoter_header: list[str]):
    ...
    # UPDATED: Audience staging will be created dynamically per file
    # (removed static causeway tables - using dynamic approach)
    
    # Note: stg_audience table will be dropped and recreated for each file
    # to accommodate different column schemas
...
# LOAD DATA
# =========================
def load_audience_file(conn, src_path: Path):
    """UPDATED: Load audience file with ALL columns, match on StateVoterId directly"""
    load_path = ensure_loadable_path(src_path, f"Audience copy {src_path.name}", COPY_LOG_EVERY_MB_CAUSEWAY)
    ...
    if "StateVoterId" not in idx:
        logger.error(f"SKIPPING {src_path.name}: Missing StateVoterId column")
        return 0
    
    # Create dynamic staging table with all columns from CSV
    ...
    col_defs.append("`audience` VARCHAR(255) NOT NULL")
    col_defs.append("KEY idx_statevoterid (StateVoterId)")
    
    create_sql = f"CREATE TABLE stg_audience ({', '.join(col_defs)}) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;"
    exec_sql(conn, create_sql)
    logger.info(f"Created staging table with {len(hdr)} columns")
    ...
    sets.append(f"audience = '{audience_escaped}'")
...
"""
    ...
    with _local_infile_progress(f"LOCAL INFILE audience {src_path.name}", LOCAL_INFILE_LOG_EVERY_MB_CAUSEWAY):
        with conn.cursor() as cur:
            cur.execute(sql)
            loaded = cur.rowcount if cur.rowcount is not None else 0
        logger.info(f"Audience load {src_path.name}: {loaded:,} rows")

    if loaded == 0:
        logger.warning(f"  WARNING: Zero rows loaded from {src_path.name}")
        
    return loaded


def load_fullvoter(conn, src_path: Path, header: list[str]):
    """UPDATED: Load ALL voters - no match key computation needed"""
...
"""
    ...
    logger.info(f"Full voter loaded successfully")
