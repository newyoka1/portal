#!/usr/bin/env python3
"""
Clean up deleted audience files from the database - AUTOMATIC VERSION
...
def main():
    ...
    try:
        ...
        if total_count == 0:
            print("\n  OK No cleanup needed - 'HT HARD DEM.csv' not found in database")
            return
        ...
        print(f"\n  Sample origin values before cleanup:")
        for (origin,) in examples[:5]:
            print(f"    - {origin}")
        ...
        # Step 2: Remove "HT HARD DEM.csv," from the beginning
        cursor.execute("""
            UPDATE voter_file
            SET origin = TRIM(SUBSTRING(origin, LENGTH('HT HARD DEM.csv,') + 1))
            WHERE origin LIKE 'HT HARD DEM.csv,%'
        """)
        step2_count = cursor.rowcount
        print(f"  Step 2: Removed from beginning of {step2_count:,} combined origins")

        # Step 3: Remove ",HT HARD DEM.csv" from the end
        cursor.execute("""
            UPDATE voter_file
            SET origin = TRIM(SUBSTRING(origin, 1, LENGTH(origin) - LENGTH(',HT HARD DEM.csv')))
            WHERE origin LIKE '%,HT HARD DEM.csv'
        """)
        step3_count = cursor.rowcount
        print(f"  Step 3: Removed from end of {step3_count:,} combined origins")

        # Step 4: Remove ",HT HARD DEM.csv," from the middle
        cursor.execute("""
            UPDATE voter_file
            SET origin = REPLACE(origin, ',HT HARD DEM.csv,', ',')
            WHERE origin LIKE '%,HT HARD DEM.csv,%'
        """)
        step4_count = cursor.rowcount
        print(f"  Step 4: Removed from middle of {step4_count:,} combined origins")

        # Step 5: Clean up double commas
        cursor.execute("""
            UPDATE voter_file
        ...
        # Show sample cleaned origins
        cursor.execute("""
            SELECT DISTINCT origin
            FROM voter_file
            WHERE origin LIKE '%HT HARD DEM INDV NYS_2555.csv%'
            LIMIT 5
        """)
        cleaned_examples = cursor.fetchall()

        if cleaned_examples:
            print(f"\n  Sample cleaned origin values:")
            for (origin,) in cleaned_examples[:5]:
                print(f"    - {origin}")
        ...
        print(f"Records affected: {total_count:,}")
