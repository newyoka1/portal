"""
Streamlit admin UI for Facebook Ads Receipt Automation.
...
st.set_page_config(page_title="FB Receipt Automation", page_icon="🧾", layout="wide")
...
with tab_clients:
    ...
    # Load all clients (active + inactive)
    ...
    # Ensure no blanks/NaN in dropdown columns so no empty option appears
    ...
    if st.button("💾 Save to Google Sheet", type="primary"):
        with st.spinner("Saving..."):
            try:
                sc = get_sheets_client()
                clients_to_save = edited_df.fillna("").to_dict("records")
                sc.save_sheet_data(settings, clients_to_save)
                st.cache_data.clear()
                st.success(f"Saved {len(clients_to_save)} clients to Google Sheet.")
            except Exception as e:
...
with tab_settings:
    ...
    with st.form("settings_form"):
        ...
        notify_email     = st.text_input("Notify email (failure alerts)",
                                          value=s.get("notify_email", "George@politikanyc.com"))
        ...
        if st.form_submit_button("💾 Save Settings", type="primary"):
            ...
            try:
                sc = get_sheets_client()
                all_clients = sc.get_all_clients_raw()
                sc.save_sheet_data(new_settings, all_clients)
                st.cache_data.clear()
    ...
    if not log_path.exists():
        ...
        try:
            ...
            if not log_data:
                ...
                # Build a flat table: one row per account
                # Try to match account IDs to client names
                try:
                    name_map = {
                ...
                h1.markdown("**Client**")
                h2.markdown("**Account ID**")
                h3.markdown("**Period**")
                h4.markdown("**Sent At**")
                h5.markdown("**Recipients**")
                h6.markdown("**Action**")
                st.divider()
